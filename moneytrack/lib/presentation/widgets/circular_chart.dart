  ...
  void calculateChartData() {
    ...
    for (var transaction in widget.transactions) {
      if (transaction.type == 'Income') {
        mapIncomeData.update(
          transaction.category.title,
          (value) => value + double.parse(transaction.amount),
          ifAbsent: () => double.parse(transaction.amount),
        );
      } else {
        mapExpenseData.update(
          transaction.category.title,
          (value) => value + double.parse(transaction.amount),
          ifAbsent: () => double.parse(transaction.amount),
        );
    ...
    return SizedBox(
      ...
      child: SfCircularChart(
        ...
        title: ChartTitle(
          text: widget.title,
          textStyle: TextStyle(
              color: widget.title == 'Income' ? Colors.green : Colors.red),
        ),
        legend: Legend(isVisible: true),
        tooltipBehavior: _tooltipBehavior,
        series: <CircularSeries>[
          DoughnutSeries<ChartData, String>(
            dataSource: widget.title == 'Income' ? incomeData : expenseData,
            xValueMapper: (ChartData data, _) => data.x,
            yValueMapper: (ChartData data, _) => data.y,
            dataLabelMapper: (ChartData data, _) =>
                '${NumberFormat.compactCurrency(symbol: '', decimalDigits: 1).format(data.y / 1000000)}M',
